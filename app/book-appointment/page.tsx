...
import { getCookie, deleteCookie, strMonth } from "../../lib/utils";
...
export default function BookAppointmentPage() {
  ...
  const renderDoctors = (doctors: any[]) => {
    ...
    grid.innerHTML = doctors.map((d: any) => `
      <div class="doctor-card" data-specialty="${d.department}" data-id="${d.id}">
        <div class="doc-card-header">
          <div class="doc-card-avatar a">${d.user.full_name[0]}</div>
          <div class="doc-card-info">
            <div class="doc-card-name">Dr. ${d.user.full_name}</div>
        ...
        <button class="select-doc-btn" data-id="${d.id}" data-name="${d.user.full_name}" data-spec="${d.specialization}" data-dept="${d.department}">Select Doctor</button>
      </div>`).join("");
  ...
  const selectDate = (dateStr: string, el: HTMLElement) => {
    ...
    if (!grid) return;
    grid.innerHTML = "";
    getSlotsForDate(dateStr).forEach((slot) => {
  ...
  const submitBooking = async () => {
    ...
    if (!response.success) { btn.disabled = false; return; }
    ...
    <div className="app">
      <aside className="sidebar">
        ...
        <nav className="nav-section">
          ...
          <Link className="nav-item" href="/user-dashboard"><svg viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></svg><span>Dashboard</span></Link>
          <Link className="nav-item" href="/my-appointments"><svg viewBox="0 0 24 24"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg><span>Appointments</span></Link>
        </nav>
      ...
      <div className="main">
        ...
        <div className="content">
          <div className="steps-bar">
            {[{n:1,label:"Choose Doctor"},{n:2,label:"Pick Date & Time"},{n:3,label:"Details"},{n:4,label:"Confirm"}].map(({n,label}) => (
              <><div key={n} className={`step${step===n?" active":step>n?" completed":""}`} data-step={String(n)}><div className="step-bubble">{n}</div><span>{label}</span></div>{n<4&&<div key={`l${n}`} className={`step-line${step>n?" completed":""}`}></div>}</>
            ))}
          </div>
